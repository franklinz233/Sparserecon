...
from sparserecon.cam_utils import orbit_camera, OrbitCamera, mat2latlon, find_mask_center_and_translate
from sparserecon.render_utils.gs_renderer import Renderer, Camera, FoVCamera, CustomCamera
from sparserecon.mesh_utils.grid_put import mipmap_linear_grid_put_2d
from sparserecon.mesh_utils.mesh import safe_normalize


class CoarseGaussianStage:
    def __init__(self, opt):
        ...
        # load input data
        self.load_views(self.opt.camera_path, self.opt.order_path)
    ...
    def prepare_optimization(self):
        ...
        if self.guidance_zero123 is None and self.enable_zero123:
            ...
            from sparserecon.guidance_utils.zero123_6d_utils import Zero123
    ...
    def load_views(self, camera_path, order_path=None):
        # load image
    ...
    @torch.no_grad()
    def export_outputs(self, mode='geo', texture_size=1024):
        ...
        if mode == 'geo':
            ...
            # self.prepare_optimization() # tmp fix for not loading 0123
            # vers = [0]
            # hors = [0]
    ...
    def run(self, iters=500):
        ...
        self.export_outputs(mode='model')
        self.export_outputs(mode='geo+tex')
...
def build_options():
    ...
    return OmegaConf.merge(OmegaConf.load(args.config), OmegaConf.from_cli(extras))


def run_from_options(opt):
    stage = CoarseGaussianStage(opt)
    stage.run(opt.iters)


def main():
    run_from_options(build_options())
